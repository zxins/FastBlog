...
def generate_origin_avatar(name, im):
    """ 生成原始大小的头像 """
    avatar_image = os.path.join(config.STATIC_FOLDER, config.AVATAR_PATH, name)
...
def new_avatar():
    common_image = os.path.join(config.STATIC_FOLDER, config.AVATAR_PATH, "avatar.jpg")
    u = uuid.uuid1()
...
def delete_avatar(name):
    """ 删除头像 """
    delete_file(config.AVATAR_PATH, name)
    delete_file(config.AVATAR_PATH, "20_20_{0}".format(name))
    delete_file(config.AVATAR_PATH, "50_50_{0}".format(name))


def change_cover(binary, cover):
    u = uuid.uuid1()
    new_cover = "{}.png".format(u.hex)
    filename = os.path.join(config.STATIC_FOLDER, config.BOOK_COVER_PATH, new_cover)
    im = Image.open(io.BytesIO(binary))
    im.save(filename)

    im = im.resize((90, 120), Image.ANTIALIAS)
    thumbnail = "thumbnail_{}".format(new_cover)
    filename = os.path.join(config.STATIC_FOLDER, config.BOOK_COVER_PATH, thumbnail)
    im.save(filename)
    if cover:
        delete_cover(cover)
    return new_cover
...
def delete_cover(cover):
    ...
    delete_file(config.BOOK_COVER_PATH, cover)
    delete_file(config.BOOK_COVER_PATH, thumbnail)


def image_url(name):
    return os.path.join("/static", config.IMAGE_PATH, name)


def enable_image(name):
    enable_tmp(config.IMAGE_PATH, name)
...
def new_tmp(file):
    ...
    path = os.path.join(config.STATIC_FOLDER, config.TMP_PATH, filename)
    file.save(path)
...
def enable_tmp(path, name):
    """ 激活临时文件 """
    filename = os.path.join(config.STATIC_FOLDER, config.TMP_PATH, name)
    if not os.path.exists(filename):
        return False
    _filename = os.path.join(config.STATIC_FOLDER, path, name)
...
def delete_file(path, name):
    """ 删除文件 """
    filename = os.path.join(config.STATIC_FOLDER, path, name)
    if not os.path.exists(filename):
        return False
    os.remove(filename)
    return True